...
function daysFromBase(days, hours = 0, minutes = 0) {
...
async function main() {
    ...
    for (let index = 0; index < 12; index += 1) {
        ...
        const organizationTypeCode = index % 4 === 0
            ? 'OWN_COMPANY'
            : index % 4 === 1
                ? 'SHIPPING_AGENT'
                : index % 4 === 2
        ...
        const location = await prisma.location.upsert({
            where: {
                code_type: {
                    code: `SOF-LOC-${serial}`,
                    type: client_1.LocationType.ANCHORAGE
                }
            },
            update: {
        ...
        const product = await prisma.product.upsert({
            ...
            create: {
                code: `SOF-PROD-${serial}`,
                name: `Bulk Cargo ${serial}`,
                type: index % 2 === 0 ? client_1.ProductType.BULK_FOOD_GRAIN : client_1.ProductType.FERTILIZER,
        ...
        const vesselCall = await prisma.vesselCall.upsert({
            ...
            update: {
                ...
                status: index % 3 === 0 ? client_1.MotherVesselStatus.DISCHARGING : client_1.MotherVesselStatus.READY_TO_DISCHARGE,
                updatedById: user.id
            },
            create: {
                callNo: `MV-ATLAS-2026-${serial}`,
        ...
        const statement = await prisma.statementOfFacts.upsert({
            ...
            update: {
                ...
                status: index % 4 === 0 ? client_1.SOFStatus.VERIFIED : client_1.SOFStatus.DRAFT,
        ...
        const tripStatusCycle = [
            client_1.LighterTripStatus.PLANNED,
            client_1.LighterTripStatus.ASSIGNED,
            client_1.LighterTripStatus.ALONGSIDE,
            client_1.LighterTripStatus.LOADING,
            client_1.LighterTripStatus.LOADED,
            client_1.LighterTripStatus.ARRIVED_GHAT
        ];
        ...
        const lighterTrip = await prisma.lighterTrip.upsert({
            ...
            create: {
                ...
                alongsideDate: daysFromBase(index, 13, 0),
                loadingStartedAt: daysFromBase(index, 14, 30),
                loadingCompletedAt: index % 4 === 0 ? daysFromBase(index, 18, 0) : undefined,
                departedMvDate: index % 5 === 0 ? daysFromBase(index, 19, 0) : undefined,
                arrivedGhatDate: index % 5 === 0 ? daysFromBase(index, 22, 0) : undefined,
                remarks: `Seed lighter trip for call ${vesselCall.callNo} · lighter ${lighterVessel.name}.`
            }
        });
        ...
        if (index % 3 === 1) {
            const ltStatement = await prisma.statementOfFacts.upsert({
                ...
                update: {
                    ...
                    status: client_1.SOFStatus.DRAFT,
    ...
    const roleRows = users.flatMap((u) => seedSofRoles.map((role) => ({ userId: u.id, role })));
    await prisma.userRoleAssignment.createMany({
        data: roleRows,
        skipDuplicates: true
    });
    await prisma.userRoleAssignment.createMany({
        data: [{ userId: users[0].id, role: client_1.AppRole.SUPER_ADMIN }],
        skipDuplicates: true
    });
    ...
    await prisma.userRoleAssignment.createMany({
        data: [{ userId: superAdminUser.id, role: client_1.AppRole.SUPER_ADMIN }],
        skipDuplicates: true
    });
    console.log(`Super admin seeded — sign in with phone ${superAdminUser.phone} (full name: ${superAdminName}). Password is set from seed script.`);
    console.log(`Demo login (includes SUPER_ADMIN): ${users[0].email} / ${demoLoginPassword}`);
    console.log(`Assigned SOF-capable roles (${seedSofRoles.join(', ')}) to ${users.length} seeded operator users.`);
